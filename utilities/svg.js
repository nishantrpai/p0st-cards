...
export function write(text, color, scale) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    letters.push(<g fill={color} transform={`translate(${letterPos},${cy})`}>{path}</g>);
    letterPos = letterPos + width + spacing;
  }

  let svg = <g transform={`scale(${scale})`} fill-rule="evenodd" clip-rule="evenodd" fill={`${color}`} aria-label={`${text}`}>${letters}</g>;
  return svg;
}